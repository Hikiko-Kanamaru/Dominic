...
public class NameListDaoImpl implements NameListDao {
	...
	public NameListDaoImpl(DataSource ds) {
		this.ds = ds;
	 ...
	 */
	@Override
	public NameListDomain insert(String name,String URL) throws Exception {

		NameListDomain nameListDomai = new NameListDomain();
		var nameOnly = findByName(name);
		if(nameOnly.getStrength()!= -1) {
			System.out.println("同一名が存在するのでここで制作しません。");
			return nameListDomai;
		}
		...
		try (Connection con = ds.getConnection()) {
			...
			String sql = "INSERT INTO `namelist` (`name`, `URL`, `Strength`) VALUES ( ?, ?, '1');";
		...
		nameListDomai = findByName(name) ;
		if (nameListDomai.getStrength() == -1) {
	...
	@Override
	public NameListDomain findByName(String name) throws Exception {
		...
		try (Connection con = ds.getConnection()) {
			...
			String sql = "SELECT * FROM namelist where name = ?;";
			PreparedStatement stmt = con.prepareStatement(sql);
			stmt.setString(1, name);
			...
			if (rs.next()) {
				...
				nameListDomai = new NameListDomain(dbid, dbname, dbURL, dbStrength) ;
	...
	@Override
	public NameListDomain findById(int id) throws Exception {
		...
		try (Connection con = ds.getConnection()) {
			...
			String sql = "SELECT * FROM namelist where id = ?;";
			PreparedStatement stmt = con.prepareStatement(sql);
			stmt.setInt(1, id);
			System.out.println("検索中1");
			ResultSet rs = stmt.executeQuery();
			...
			System.out.println("findByIdサーバー接続に失敗しました");
			throw e;
		}
	...
	@Override
	public NameListDomain findById(int id,Connection con) throws Exception {
	...
	@Override
	public NameListDomain update(NameListDomain domain) throws Exception {
		...
		try (Connection con = ds.getConnection()) {
			...
			String sql = "UPDATE `namelist` SET `URL` = ?, `Strength` = ? WHERE (`id` = ?);";
	 ...
	 */
	@Override
	public NameListDomain insertorUpdate(String name, String URL) throws Exception {

		NameListDomain nameListDomain = findByName(name);

		if(nameListDomain.getStrength() == -1) {
//			インサート処理
			nameListDomain = insert(name, URL);

		}else {
//			アップデート処理
			nameListDomain = update(nameListDomain);
		}
